...
int main(){
	...
	Carro novo, saidav1[4], saidav2_1, saidav2_2, saidaes, saidaen1, saidaen2;
	...
	char t = '&';
	Carro* subst = (Carro*) malloc(sizeof(Carro)*4);
	...
	int mark2_f = mark2;
		...
		while(mark2 < strlen(lista_v) || Qtecarros(faixa_1) != 0 || Qtecarros(faixa_2) != 0 || Qtecarros1(via1, 1) != 0 || Qtecarros1(via1, 2) != 0 || ES->vet[ES->topo].tipov != ' '){
			...
			if (saidav1[2].tipov != ' '){
				fprintf(saida, "%c %d %d\n", saidav1[2].tipov, saidav1[2].nveiculo, ciclo);
			}
			if (saidav1[3].tipov != ' '){
				fprintf(saida, "%c %d %d\n", saidav1[3].tipov, saidav1[3].nveiculo, ciclo);
			}
			if (saidav2_1.tipov != ' '){
